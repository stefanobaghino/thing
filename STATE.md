...
- 79 builtins; thirteen embedded stdlib modules
  ...
  CI platforms, Windows included); 534 Rust tests
...
- 917: second stroke — THE KEY YOU TAKE OUT. `pop(m, k)` removes a
  ...
  every refusal. The NAME is the design: `pop(xs)` already removes
  and returns the last element, and ting overloads across types
  (len/find/contains/slice) and arities (run/try/input/read_file)
  already. A missing key ERRORS, with the same "did you mean" message
  indexing gives — the suggestion moved into `eval::key_miss`, shared
  by reading a key and taking one out. Draining 4000 keys went from
  8431 ms to 5 ms; 1000 to 32000 keys is linear at 1/2/5/10/21/42 ms.
  The test also pins that `m[k] = nil` KEEPS the key and pop does not.
...
- 933: columns where a column is meant — format's width specs,
  lib/string's pad_left/pad_right/center/table/wrap, truncate through
  a new fit(s, width), the --doc wrapper and the :help signature
  column (both had been measuring BYTES). Six sites, each
...
- 988: `--doc <module>` lists members in definition order, so six
  modules that opened with a helper (args, base64, csv, sh, test,
  time) now open with what their docs/stdlib.md table opens with;
  args's and map's tables moved to agree. A top-level function may
...
- 1003: after an exact hit `--doc`/`:doc` name what else mentions the
...
- 1012: v2.155.0 VERIFIED — seven assets, six checksums OK cold, both
...
- 1017: `{"show": true}` lets a child write to ting's own streams as
  ...
  tests, 3007 checks.
- 1018: v2.156.0 tagged (177th tag; strokes 1015, 1016, 1017).
- 1019: v2.156.0 VERIFIED — seven assets, six checksums OK cold, both
...
- v2.157.0 VERIFIED (178th tag; strokes 1022, 1023, 1024; both
  aarch64 archives executed here).
- 1027: health tick + audit green — eleven bench checksums identical
  to BASELINE, four sweeps ok, counts all standing (79 builtins, 13
...
- v2.158.0 VERIFIED (179th tag; strokes 1029, 1030, 1031; both
  aarch64 archives executed here).
- 1034: health tick + audit green — eleven bench checksums identical
  to BASELINE, four sweeps ok, counts all standing (79 builtins, 13
...
- 1037: `--check` reads a literal format template with
  `eval::format_trouble` and answers in the run's own words; a test
  runs both over thirteen calls. 502 tests.
- 1038: a run that stops on a shadowed builtin says so — `map is not
...
- v2.159.0 VERIFIED (180th tag; strokes 1036, 1037, 1038; both
  aarch64 archives executed here).
- 1041: health tick + audit green — eleven bench checksums identical
  to BASELINE, four sweeps ok, counts all standing (79 builtins, 13
...
- 1054: a type annotation — `:` where a `)`, `{` or `=` was wanted,
  or Rust's `->` — says ting has none and names `type(v)`. A Python
  slice keeps the plain message. 522 tests.
...
- 1064: the timing guards take their bound and stop rounding as soon
  ...
  none a regression.
- 1065: v2.162.0 tagged (183rd tag; strokes 1060, 1061, 1062, 1063,
  1064).
...
- 1069: `csv["rows"](records, columns)`, the inverse of `maps` —
  ...
  3088 checks.
- 1070: `csv["text"]` refuses a record row, naming `rows`, and any
  row that is not a list — it used to write a record's keys. 3093
  checks.
...
- v2.163.0 VERIFIED (184th tag; seven assets, six sums OK, both
  aarch64 archives executed here, ten paths 200).
- 1075: health tick — eleven checksums identical to BASELINE (and
  the timings within a few per cent this time), four sweeps ok,
  counts all standing (79 builtins, 13 modules, 231 functions, 50
...
- v2.164.0 VERIFIED (185th tag; seven assets, six sums OK, both
  ...
  paths 200).
- 1085: health tick — eleven checksums identical to BASELINE, four
  sweeps ok, counts all standing (79 builtins, 13 modules, 231
...
- 1103: no length floor on `Found::Inside` — a whole part is
  ...
  finds nothing.
- 1104: `crate::diag::cannot_read` — text where a path belongs is
  diagnosed and cut to thirty characters; a real path keeps its
  whole name.

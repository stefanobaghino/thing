...
| `table(rows, align = "")` | rows of strings padded into aligned columns, two spaces apart; `align` is one of `<`, `>` or `^` per column, the rest left. Rows and cells are checked first: a row must be a list and a cell a string, and a record says to pass `rows(records)` from lib/csv.ting |
...
| `check_err(name, f, want)` | passes if `f()` fails with a message containing `want`; a wrong message names the line that raised it |
